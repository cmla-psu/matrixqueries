...
def basic_l2_privacy_cost_vector(strategy: np.ndarray, noise_scale: float) -> np.ndarray:
...
def l2_privacy_cost_vector(strategy: np.ndarray, invcov: np.ndarray = None, cov: np.ndarray = None) -> np.ndarray:
    """ Returns the privacy cost vector of the strategy matrix when multivariate Gaussian noise is added

    Input:
       strategy: the query matrix to which noise would be added in a dp mechanism
       invcov: the inverse of the covariance matrix
       cov: the positive definite covariance matrix
...
def l2_privacy_cost(strategy: np.ndarray, invcov: np.ndarray = None, cov: np.ndarray = None) -> float:
    """ Returns the privacy cost of the strategy matrix
    when iid noise is added, this is the same as the largest of the l2 column norms
